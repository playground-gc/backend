...
async def process_trade(conn: asyncpg.Connection, trade: dict) -> None:
    symbol        = trade["symbol"]
    ...
    if buy_order_id:
        await conn.execute(
            """
            UPDATE orders
            SET filled_qty = LEAST(filled_qty + $2, quantity),
                status     = CASE
                    ...
                    try:
                        ...
                        async with pg_pool.acquire() as conn:
                            async with conn.transaction():
                                await process_trade(conn, trade)
                        log.info(
                        ...
                        log.error("Error processing trade entry %s: %s", entry_id, e)
